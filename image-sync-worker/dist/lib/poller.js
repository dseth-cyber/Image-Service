import { SmbClient, buildSmbPath } from './smb.js';
import { scanCameraDirectory } from './scanner.js';
import { computeChecksum } from './checksum.js';
import * as api from './api-client.js';
import { logger } from './logger.js';
import { config } from '../config/index.js';
const cameraPollIntervals = new Map();
export class CameraPoller {
    tracker;
    producer;
    constructor(tracker, producer) {
    ...
    async pollAllCameras() {
        ...
        try {
            ...
            const pollPromises = activeCameras.map((camera) => this.pollSingleCamera(camera).catch((err) => {
            ...
            logger.info({ cameras: results.length, newFiles: totalNew, errors: totalErrors }, 'Poll cycle complete');
        }
        catch (err) {
    ...
    async pollSingleCamera(camera) {
        ...
        try {
            ...
            for (const file of files) {
                const filePath = buildSmbPath(camera, file.originalFilename);
                let checksum;
                try {
                ...
                if (alreadyProcessed) {
                    logger.debug({ camera: camera.name, file: file.originalFilename, checksum: checksum.slice(0, 16) }, 'Duplicate file, skipping');
                    stats.duplicates++;
                    continue;
                }
                toProcess.push({ file, checksum });
            }
            logger.info({ camera: camera.name, scanned: stats.scanned, toProcess: toProcess.length, duplicates: stats.duplicates }, 'Scan complete');
            for (const { file, checksum } of toProcess) {
                try {
                    ...
                    stats.newFiles++;
                    logger.info({ camera: camera.name, file: file.originalFilename, imageId }, 'File queued for processing');
                }
                catch (err) {
                    logger.error({ camera: camera.name, file: file.originalFilename, err }, 'Failed to process file');
                    stats.errors++;
                }
            }
            await api.updateCameraPoll(camera.id, {
            ...
            stats.errors++;
            await api.reportError(camera.id, `Scan failed: ${err.message}`);
        }
        finally {
            await smb.close();
        }
    ...
    buildResult(camera, stats, startTime) {
        return {
            cameraId: camera.id,
            cameraName: camera.name,
            scanned: stats.scanned,
...
function updateCameraInterval(camera) {
    cameraPollIntervals.set(camera.id, Date.now());
}
export function resetCameraInterval(cameraId) {
