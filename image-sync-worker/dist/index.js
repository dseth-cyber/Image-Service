...
async function main() {
    logger.info({ nodeEnv: config.nodeEnv, pollIntervalMs: config.polling.intervalMs }, 'Starting image-sync-worker');
...
async function handleShutdown(redis, producer, healthServer) {
    if (shutdownRequested)
        return;
