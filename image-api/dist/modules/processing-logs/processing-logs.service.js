...
export async function searchProcessingLogs(params) {
    ...
    const data = rows.map((row) => {
        ...
        return {
            id: r.id,
            imageId: r.imageId,
            imageFilename: r.image?.originalFilename ?? null,
            jobType: r.jobType,
            workerId: r.workerId,
            status: r.status,
            priority: r.priority,
...
export async function bulkRetryDlq(jobType) {
    const prisma = getPrisma();
...
export async function bulkRejectDlq(jobType) {
    const prisma = getPrisma();
    const where = { status: 'dead_letter' };
    if (jobType)
        where.jobType = jobType;
...
//# sourceMappingURL=processing-logs.service.js.map