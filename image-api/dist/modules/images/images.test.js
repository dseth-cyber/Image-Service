...
describe('ImagesService', () => {
    ...
    describe('getImageById', () => {
        ...
        it('should throw NotFoundError for deleted image', async () => {
            ...
            await expect(imagesService.getImageById('img-1')).rejects.toThrow(NotFoundError);
        });
        it('should throw NotFoundError for missing image', async () => {
            mockPrisma.image.findUnique.mockResolvedValue(null);
            await expect(imagesService.getImageById('img-missing')).rejects.toThrow(NotFoundError);
        });
    });
    describe('updateImageMetadata', () => {
        it('should update metadata fields', async () => {
            ...
            expect(result).toBeDefined();
            expect(mockPrisma.image.update).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: 'img-1' },
        ...
        it('should throw on missing image', async () => {
            mockPrisma.image.findUnique.mockResolvedValue(null);
            await expect(imagesService.updateImageMetadata('img-missing', {})).rejects.toThrow(NotFoundError);
        });
    });
    describe('softDeleteImage', () => {
        ...
        it('should throw on already deleted image', async () => {
            ...
            await expect(imagesService.softDeleteImage('img-1')).rejects.toThrow(NotFoundError);
        });
    });
});
