...
export async function runMinioBackup() {
    const prisma = getPrisma();
    const record = await prisma.backupRecord.create({
...
export async function runRestoreTest(backupRecordId) {
    const prisma = getPrisma();
...
export async function getBackupStatus() {
    ...
    for (const s of stats) {
        if (!byType[s.type])
            byType[s.type] = { total: 0, completed: 0, failed: 0, running: 0 };
        byType[s.type].total += s._count.id;
        if (s.status === 'completed')
            byType[s.type].completed += s._count.id;
        else if (s.status === 'failed')
            byType[s.type].failed += s._count.id;
        else if (s.status === 'running')
            byType[s.type].running += s._count.id;
    }
    return {
        database: latestDb ? {
            id: latestDb.id,
